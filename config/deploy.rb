...
set :deploy_to, "/home/bourouch/capistrano_deploy_test/#{fetch(:application)}"
...
task :smoke_test do
  on roles(:app) do
    ...
    failed_tests = []
    curl_result = `ls #{fetch(:deploy_to)}`
    failed_tests << 'smoke test FAILURE.' unless (curl_result != '')
...
#   end
  ...
  task :restart do
    on roles(:app), in: :sequence, wait: 5 do
      ...
      execute 'ls /home/bourouch/capistrano_deploy_test'
      execute 'sleep 5'
    end
  end
